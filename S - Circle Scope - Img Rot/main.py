...
def draw(screen, eyesy):
    global xr, yr, lx, ly, rotation_angle, color
    eyesy.color_picker_bg(eyesy.knob5)
    color = eyesy.color_picker_lfo(eyesy.knob4)
    rotation_angle += 0.65  # Increment the rotation angle for constant spinning
    for i in range(0, 50):
...
def seg(screen, eyesy, i, angle):
    ...
    screen.blit(image, (rotated_x - (image_width / 2), rotated_y - (image_height / 2)))
